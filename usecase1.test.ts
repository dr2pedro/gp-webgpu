...
import { Buffer, Command } from "./index.ts";
...
const writeBuffer = new Buffer(device).create(4, GPUBufferUsage.MAP_WRITE | GPUBufferUsage.COPY_SRC, true, 'write-buffer');
const arrayBuffer = writeBuffer.getMappedRange();
new Uint8Array(arrayBuffer).set([0, 1, 2, 3]);
writeBuffer.unmap();

const readBuffer = new Buffer(device).create(4, GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ);
